...
function getPythonExecutable(): string {
  ...
  const prodExe =
    process.platform === 'win32'
      ? path.join(process.resourcesPath, 'backend_dist', 'jmcomic_backend', 'jmcomic_backend.exe')
      : path.join(process.resourcesPath, 'backend_dist', 'jmcomic_backend', 'jmcomic_backend');
  return prodExe;
}
...
export async function startBackend(options: BackendLauncherOptions): Promise<ChildProcess> {
  ...
  // Prod: PyInstaller onedir executable handles the same args.
  const args = ['-m', 'jmcomic_backend.main', '--host', '127.0.0.1', '--port', String(port), '--data-dir', dataDir];
...
export async function waitForBackend(port: number, timeoutMs = 30000): Promise<void> {
