...
// Windows 下 Node 不做 PATHEXT 解析，spawn 无扩展名的 .bin 入口会 ENOENT，
// 必须显式用 .cmd 版本
const binExt = process.platform === "win32" ? ".cmd" : "";
const binDir = path.resolve(__dirname, "..", "node_modules/.bin");
const tscBin = path.join(binDir, `tsc${binExt}`);
const electronBin = path.join(binDir, `electron${binExt}`);
...
async function main() {
  ...
  if (build.error) {
    // spawn 失败（如 Windows 下 bin 缺失）时 error 有值、status 为 null，
    // 直接退出会把真实原因吞掉
    console.error(`无法运行 tsc（${tscBin}）: ${build.error.message}`);
    process.exit(1);
  }
  if (build.status !== 0) {
  ...
  try {
    ...
    electron = spawn(electronBin, [".", "--remote-debugging-port=9223"], {
      stdio: "inherit",
      env: {
        ...process.env,
